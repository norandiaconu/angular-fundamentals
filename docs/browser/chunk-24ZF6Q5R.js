...
var AsyncScheduler = class _AsyncScheduler extends Scheduler {
  ...
  schedule(work, delay2 = 0, state) {
    if (_AsyncScheduler.delegate && _AsyncScheduler.delegate !== this) {
      return _AsyncScheduler.delegate.schedule(work, delay2, state);
    } else {
      return super.schedule(work, delay2, state);
    }
  }
  flush(action) {
    const { actions } = this;
...
function dispatch(state) {
  const { subscriber, counter: counter2, period } = state;
  subscriber.next(counter2);
  this.schedule({ subscriber, counter: counter2 + 1, period }, period);
}
...
function merge(...observables) {
  ...
  let scheduler = null;
  let last2 = observables[observables.length - 1];
...
function throwError(error, scheduler) {
  if (!scheduler) {
    ...
    return new Observable((subscriber) => scheduler.schedule(dispatch4, 0, { error, subscriber }));
  }
}
function dispatch4({ error, subscriber }) {
...
var AuditSubscriber = class extends OuterSubscriber {
...
var DelaySubscriber = class _DelaySubscriber extends Subscriber {
  ...
  static dispatch(state) {
    ...
    if (queue.length > 0) {
      const delay2 = Math.max(0, queue[0].time - scheduler.now());
      this.schedule(state, delay2);
    } else {
      this.unsubscribe();
      source.active = false;
    }
...
var ExhaustMapSubscriber = class extends OuterSubscriber {
...
var MapToSubscriber = class extends Subscriber {
...
var SampleSubscriber = class extends OuterSubscriber {
  constructor() {
    super(...arguments);
    this.hasValue = false;
  }
...
var SampleTimeSubscriber = class extends Subscriber {
...
function throttleTime(duration, scheduler = async, config2 = defaultThrottleConfig) {
  return (source) => source.lift(new ThrottleTimeOperator(duration, scheduler, config2.leading, config2.trailing));
}
var ThrottleTimeOperator = class {
...
var ThrottleTimeSubscriber = class extends Subscriber {
